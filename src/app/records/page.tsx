...
const patientRecords = [
  ...
  },
  {
    ...
    date: "2024-07-10",
    details: "Follow-up with Dr. Reddy",
    status: "Available",
  },
  {
    id: "REC-003",
...
export default function RecordsPage() {
  ...
  const [consentStatus, setConsentStatus] = React.useState<
    "pending" | "granted" | "denied" | "request_sent"
  >("pending");
  const [consentAction, setConsentAction] = React.useState<"view" | "upload">(
    "view"
  );
  ...
  const [isDoctorUploadModalOpen, setIsDoctorUploadModalOpen] =
    React.useState(false);
  const [doctorUploadDocName, setDoctorUploadDocName] = React.useState("");
  const [doctorUploadFile, setDoctorUploadFile] = React.useState<File | null>(
    null
  );
  ...
  const handleGrantConsent = () => {
    // This would be called by the patient
  ...
  const renderDoctorView = () => (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Enter a unique patient ID to find their records.
        </CardDescription>
      </CardHeader>
      <CardContent>
        ...
        {searched && foundPatient && (
          <Card className="bg-muted/50">
            <CardHeader>
              ...
              <CardDescription>
                ID: {foundPatient.id} | DOB: {foundPatient.dob}
              </CardDescription>
            </CardHeader>
            <CardContent className="flex justify-end gap-2">
              <Button
                ...
                onClick={() => {
                  ...
                  setIsDoctorUploadModalOpen(true);
                }}
              >
                <FileUp className="mr-2" /> Upload for Patient
              </Button>
              <Button onClick={() => handleViewRecords(foundPatient)}>
                View Records
              </Button>
            </CardContent>
          </Card>
    ...
    <div className="space-y-8">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Here is a list of your available medical records.
          </CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Add a new document to your personal records.
          </CardDescription>
        </CardHeader>
        <CardContent>
  ...
  return (
    <div className="space-y-8">
      ...
      {hasAccess && selectedPatient ? (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Access granted. Patient ID: {selectedPatient.id}
            </CardDescription>
          </CardHeader>
      ...
      <Dialog open={isConsentModalOpen} onOpenChange={setIsConsentModalOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              To {consentAction} records for {selectedPatient?.name}, patient
              consent is required.
            </DialogDescription>
          </DialogHeader>
          ...
          {consentStatus === "request_sent" && (
            <div className="flex flex-col items-center justify-center space-y-4 py-8 text-center">
              ...
              <p className="text-muted-foreground">
                Your request has been sent to the patient for approval.
              </p>
            </div>
          )}
          {consentStatus === "denied" && (
            <div className="flex flex-col items-center justify-center space-y-4 py-8 text-center">
              ...
              <p className="text-muted-foreground">
                The transaction was rejected. You cannot access these records.
              </p>
            </div>
          )}
      ...
      <Dialog
        open={isDoctorUploadModalOpen}
        onOpenChange={setIsDoctorUploadModalOpen}
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              Upload Document for {selectedPatient?.name}
            </DialogTitle>
            <DialogDescription>
              Select a file and provide a name. This will require the patient's
              consent.
            </DialogDescription>
          </DialogHeader>
          <form onSubmit={handleDoctorUploadRequest} className="space-y-4 py-4">
            ...
            <DialogFooter>
              <Button
                type="button"
                variant="outline"
                onClick={() => setIsDoctorUploadModalOpen(false)}
              >
                Cancel
              </Button>
